...
void const* gpu_computeVertexAttributeDataPointer(
  ...
  //return head->buffer + head->offset + gl_VertexID * head->stride; ||||||||||||||||||||||||||||||
...
void gpu_runVertexPuller(
  ...
  for (int x = 0; x < MAX_ATTRIBUTES; x++) {
      
      if (&puller->heads[x] != NULL && (puller->heads[x].enabled != 0)) {
          output->attributes[x] = gpu_computeVertexAttributeDataPointer(&puller->heads[x], gpu_computeGLVertexID(puller->indices, vertexShaderInvocation));
      }
      else {
          output->attributes[x] = NULL;
      }
  }
...
void gpu_runPrimitiveAssembly(
  ...
  GPUVertexPullerOutput pullerOut; 
  GPUVertexShaderInput shaderIn;
  
  for (unsigned int x = 0; x < nofPrimitiveVertices; x++) {
      
      gpu_runVertexPuller(&pullerOut, puller, baseVertexShaderInvocation + x);
      shaderIn.gl_VertexID = gpu_computeGLVertexID(puller->indices, baseVertexShaderInvocation + x);
      shaderIn.attributes = (const GPUVertexPullerOutput *) pullerOut.attributes;
      vertexShader(&primitive->vertices[x], &shaderIn, gpu);
  }
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  Vec3 vecAP, vecAC, vecAB, areaAPC, areaAPB, areaABC;
  Vec2 tmp;
  float zCord;
  (void) lines;

  // create vector AB
  sub_Vec2(&tmp, &vertices[1], &vertices[0]);
  init_Vec3(&vecAB, tmp.data[0], tmp.data[1], 0.0);
  
  // create vector AC
  sub_Vec2(&tmp, &vertices[2], &vertices[0]);
  init_Vec3(&vecAC, tmp.data[0], tmp.data[1], 0.0);
  
  // create vector AP
  sub_Vec2(&tmp, pixelCenter, &vertices[0]);
  init_Vec3(&vecAP, tmp.data[0], tmp.data[1], 0.0);
  
  // calculate area of APB as the z-coordinate of cross product of AP and AB
  zCord = vecAP.data[0] * vecAB.data[1] - vecAP.data[1] * vecAB.data[0];
  init_Vec3(&areaAPB, 0, 0, zCord);

  // calculate area of Apc as the z-coordinate of cross product of AP and AC
  zCord = vecAP.data[0] * vecAC.data[1] - vecAP.data[1] * vecAC.data[0];
  init_Vec3(&areaAPC, 0, 0, zCord); 

  // calculate area of ABC as the z-coordinate of cross product of AB and AC
  zCord = vecAC.data[0] * vecAB.data[1] - vecAC.data[1] * vecAB.data[0];
  init_Vec3(&areaABC, 0, 0, zCord); 

  coords->data[2] = (float) fabs((areaAPB.data[2]/2) / (areaABC.data[2]/2));
  coords->data[1] = (float) fabs((areaAPC.data[2]/2) / (areaABC.data[2]/2));
