...
**/
	...
	else if ((*RootPtr) != NULL)
	{
		...
		tBTNodePtr tempAid = *RootPtr;

		while (TRUE)
		{
		
			if (tempAid->Cont == Content)
			{
				return;
			}
			else if ((tempAid->Cont > Content) && (tempAid->LPtr == NULL))
			{
				tempAid->LPtr = malloc(sizeof(struct tBTNode));
				
				if (tempAid->LPtr == NULL)
					return;

				tempAid->LPtr->Cont = Content;
				tempAid->LPtr->LPtr = NULL;
				tempAid->LPtr->RPtr = NULL;

				return;
			}
			else if ((tempAid->Cont < Content) && (tempAid->RPtr == NULL))
			{
				tempAid->RPtr = malloc(sizeof(struct tBTNode));
				if (tempAid->RPtr == NULL)
					return;

				tempAid->RPtr->Cont = Content;
				tempAid->RPtr->LPtr = NULL;
				tempAid->RPtr->RPtr = NULL;

				return;
			}
			else if (tempAid->Cont < Content)
				tempAid = tempAid->RPtr;
			
			else if (tempAid->Cont > Content)
				tempAid = tempAid->LPtr;
...
**/
	...
	while(TRUE)
	{
		if(SEmptyP(&s) == TRUE)
			return;
		Leftmost_Preorder(STopPopP(&s)->RPtr, &s);
	}
	return;
	 //solved = FALSE;		  /* V případě řešení smažte tento řádek! */	
}
...
**/
	...
	while(TRUE)
	{
		if ((SEmptyP(&sp) == TRUE) || (SEmptyB(&sb) == TRUE))
			return;
